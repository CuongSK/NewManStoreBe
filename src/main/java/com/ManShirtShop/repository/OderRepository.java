...
public interface OderRepository extends JpaRepository<Order, Integer> {
        ...
        @Query(value = "select orders.code as 'id',orders.total as 'totalOrder',sum(Case when returns.status = 2 then returns.total else 0 end) as 'totalReturn'\n" + //
...
//        Integer checkIfOrderUpdatedMoreThan7DaysAgo(Integer orderId);
        // @Query(value = "SELECT CASE WHEN orders.update_time is not null THEN " +
        // "datediff(Date(CONVERT_TZ(now(), '+00:00', '+07:00')), Date(orders.update_time)) " +
        // "ELSE -1 END " +
        // "FROM orders " +
        // "WHERE orders.id = ?1", nativeQuery = true)
        ...
        @Query(value = "select returns.code from returns \n" + //
                "join return_detail on returns.id = return_detail.return_id\n" + //
                "join order_details on return_detail.order_detail_id = order_details.id\n" + //
                "join orders on order_details.order_id = orders.id\n" + //
                "where returns.status = 2 and orders.code= :code"
        , nativeQuery = true)
        String getCodeReturn(String code); 
        @Query(value = "select Month(orders.update_time) as 'month',count(orders.id) as 'count' from \n" + //
                    "orders where orders.status = 4 and Year(orders.update_time) = :year group by Month(orders.update_time)"
        , nativeQuery = true)
        List<CountorderDto> getOrderCancelByMonth(Integer year); 
        @Query(value = "select Month(returns.update_time) as 'month',count(returns.id) as 'count' from \n" + //
                    "returns where returns.status = 2 and Year(returns.update_time) = :year group by Month(returns.update_time)"
        , nativeQuery = true)
        List<CountorderDto> getReturnSuccessByMonth(Integer year);

        @Query(value = "select orders.code from returns \n" +
                "join return_detail on returns.id = return_detail.return_id\n" +
                "join order_details on return_detail.order_detail_id = order_details.id\n" +
                "join orders on order_details.order_id = orders.id\n" +
                "where returns.code= :codeRe", nativeQuery = true)
        Optional<String> getCodeOrderByCodeReturn(@Param("codeRe") String codeRe);

        @Query(value =  "select orders.ship_phone from returns \n" +
                "join return_detail on returns.id = return_detail.return_id\n" +
                "join order_details on return_detail.order_detail_id = order_details.id\n" +
                "join orders on order_details.order_id = orders.id\n" +
                "where returns.code= :codeRe", nativeQuery = true)
        Optional<String> getSdtByCodeReturns(@Param("codeRe") String codeRe);
}
