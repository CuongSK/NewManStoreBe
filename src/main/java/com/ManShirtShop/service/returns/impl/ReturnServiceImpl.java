...
//        }
        ...
        aReturn.setStatus(ReturnsContact.STATUS_THANH_CONG); // Mặc định status yêu cầu trả hàng
...
//                    System.out.println(orderDetail1.get().getId());
                    ...
                    returnDetail.setStatus(ReturnsContact.STATUS_THANH_CONG);
...
//        if ((aReturn.getStatus() == ReturnsContact.St) || (aReturn.getStatus() == ReturnsContact.STATUS_DA_HUY)) {
//            throw new IllegalArgumentException("Return with id " + returnId + " has already been processed.");
//        }
...
//        }
    ...
    public ReturnResponseAdminGetAll findReturnAndDetailByCode(String code) {
        ...
        Optional<String> codeOrder = orderRepository.getCodeOrderByCodeReturn(returnResponseAdminGetAll.getCode());
        Optional<String> sdt = orderRepository.getSdtByCodeReturns(returnResponseAdminGetAll.getCode());
        if (codeOrder.isPresent()){
            returnResponseAdminGetAll.setCodeOrder(codeOrder.get());
        }
        if (sdt.isPresent()){
            returnResponseAdminGetAll.setSdt(sdt.get());
        }
